  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (isLogin) {
        ...
        if (res?.success) {
          toast.success("Successfylly Registered");
      ...
      >
        ...
        <div>
          <Button
            ...
            variant={"outline"}
            className="text-lg font-bold w-full py-7 rounded-xl cursor-pointer"
          >
            {" "}
            <FcGoogle /> গুগল দিয়ে লগিন
          </Button>
        </div>
      </motion.div>
    </div>
