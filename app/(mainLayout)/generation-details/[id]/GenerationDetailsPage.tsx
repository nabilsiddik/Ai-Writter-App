...
export default function GenerationDetails({
  ...
  const handleOpenInGoogleDoc = async () => {
    ...
    try {
      ...
      toast.error("Google Docs Error");
    } finally {
      setGoogleDocGenerating(false);
    }
  };
  ...
  const handleExportInMsWord = async () => {
    ...
    try {
      ...
      console.log(err, "Error while word export");
      toast.error("Word Export Error");
    } finally {
      setMsDocxGenerating(false);
    }
  };
  ...
  const handleUploadToWoocommerce = async () => {
    ...
    try {
      ...
      toast.error("WooCommerce API Error");
    } finally {
      setUploadingWoo(false);
    }
  };
  ...
  const inputStyles =
    "w-full bg-white border-2 border-slate-100 rounded-xl px-6 py-4 text-lg text-black placeholder:text-slate-300 focus:outline-none focus:border-primary transition-all";
        ...
        {isGoogleModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              className="relative w-full max-w-xl bg-white border border-slate-200 rounded-3xl p-12 shadow-2xl overflow-hidden text-center"
            >
              ...
              <p className="text-slate-500 text-xl leading-relaxed mb-10">
                Grant permission to save documents directly to your Google
                Workspace.
              </p>
              <button
                onClick={handleGoogleLogin}
                className="w-full py-5 bg-primary text-white rounded-xl font-bold text-xl hover:bg-indigo-700 transition-all cursor-pointer"
              >
        ...
        {isWooModalOpen && (
          <div className="fixed inset-0 z-[100] flex items-center justify-center p-6">
            ...
            <motion.div
              ...
              animate={{ opacity: 1, scale: 1 }}
              exit={{ opacity: 0, scale: 0.95 }}
              className="relative w-full max-w-2xl bg-white border border-slate-200 rounded-[32px] p-10 shadow-2xl overflow-hidden"
            >
              <button
                onClick={() => setIsWooModalOpen(false)}
                className="absolute top-6 right-6 text-slate-400 hover:text-black cursor-pointer"
              >
                <X size={28} />
              </button>
              <div className="flex items-center gap-4 mb-8">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-lg font-medium">
                    Link your WooCommerce marketplace
                  </p>
                </div>
              </div>
              ...
              <button
                ...
                className="w-full py-5 bg-primary text-white rounded-xl font-bold text-xl hover:bg-indigo-700 transition-all flex items-center justify-center gap-3 cursor-pointer shadow-lg shadow-primary/20"
              >
                {isConnecting ? (
        ...
        <div className="max-w-7xl mx-auto flex flex-col md:flex-row justify-between items-center gap-8">
          <div className="text-center md:text-left">
            ...
            <p className="text-slate-500 text-xl font-medium">
              Refine your document and choose a professional format.
            </p>
          </div>
          <div className="flex flex-wrap items-center justify-center gap-4">
            <button
              ...
              className="flex items-center gap-3 px-6 py-4 bg-white border border-slate-200 rounded-xl font-bold hover:shadow-md transition-all cursor-pointer"
            >
              {pdfGenerating ? (
      ...
      <main className="max-w-7xl mx-auto px-6 py-16">
        <div className="flex items-center gap-4 mb-12">
          <button
            ...
            className="flex items-center gap-2 text-slate-400 hover:text-black font-bold transition-all cursor-pointer"
          >
        ...
        <motion.section
          ...
          initial="hidden"
          animate="visible"
          className="grid grid-cols-1 lg:grid-cols-3 gap-12 mb-20 bg-slate-100 p-10 rounded-3xl border border-slate-300"
        >
        ...
        <section className="space-y-8 mb-24">
          <div className="flex items-center gap-4 mb-6">
            <h3 className="text-xl font-black">Document Sections</h3>
          ...
          <div className="grid gap-8">
            {editableSections?.map((sec: any, idx: number) => (
              <motion.div
                ...
                whileInView="visible"
                viewport={{ once: true }}
                className="bg-white border border-slate-200 rounded-[24px] overflow-hidden hover:shadow-lg transition-all"
              >
                <div className="bg-slate-50/50 p-8 border-b border-slate-100">
                  <input
                    className="w-full bg-transparent text-xl lg:text-2xl font-black text-black focus:outline-none"
                    value={sec?.topic || ""}
                    onChange={(e) =>
                      handleSectionUpdate(idx, "topic", e.target.value)
                    }
                  />
                </div>
                <div className="p-8">
                  <textarea
                    rows={5}
                    className="w-full bg-transparent text-xl text-slate-600 leading-relaxed focus:outline-none resize-none"
                    value={sec?.content || ""}
                    onChange={(e) =>
                      handleSectionUpdate(idx, "content", e.target.value)
                    }
                  />
                </div>
              </motion.div>
        ...
        {document?.docType === "PRODUCT_DESC" && (
          <motion.section
            ...
            className="mb-24 p-12 bg-slate-50 border border-slate-200 rounded-[40px] shadow-sm"
          >
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-10 mb-12">
              ...
              <div>
                <label className={labelStyles}>Stock Level</label>
                <input
                  type="number"
                  className={inputStyles}
                  placeholder="0"
            ...
            {wooStore ? (
              <button
                ...
                disabled={uploadingWoo}
                className="w-full py-6 bg-emerald-600 text-white rounded-2xl font-bold text-xl hover:bg-emerald-700 transition-all flex items-center justify-center gap-3 cursor-pointer shadow-lg shadow-emerald-600/20"
              >
                {uploadingWoo ? (
              ...
              <button
                ...
                className="w-full py-6 bg-slate-900 text-white rounded-2xl font-bold text-xl hover:bg-black transition-all flex items-center justify-center gap-3 cursor-pointer"
              >
        ...
        <motion.div
          initial={{ opacity: 0 }}
          whileInView={{ opacity: 1 }}
          className="text-center bg-slate-50 border border-slate-200 p-16 rounded-[48px]"
        >
