    ...
    <div className="pb-20">
      <header className="mb-12">
        ...
        <p className="text-xl text-slate-500 font-medium leading-relaxed">
          Manage your community, monitor plans, and moderate accounts.
        </p>
      </header>
            ...
            <tbody className="divide-y divide-slate-100">
              {users?.map((user: any, index: number) => (
                ...
                >
                  ...
                  <td className="p-8">
                    <div
                      className={`inline-flex items-center gap-2 px-4 py-2 rounded-xl text-base font-black uppercase tracking-widest ${
                        user?.plan === "PREMIUM"
                    ...
                    {user?.subscription?.endDate && (
                      <p className="text-base text-slate-400 mt-2 font-bold italic">
                        Ends:{" "}
                        {format(
                  ...
                  <td className="p-8">
                    <div
                      className={`flex items-center gap-2 font-black text-lg ${user?.status === "ACTIVE" ? "text-emerald-500" : "text-red-500"}`}
                    >
                      <div
                        className={`w-2.5 h-2.5 rounded-full animate-pulse ${user?.status === "ACTIVE" ? "bg-emerald-500" : "bg-red-500"}`}
                      />
                      {user?.status}
                    </div>
                  </td>
                  ...
                  <td className="p-8">
                    <div className="flex items-center justify-center gap-3">
                      ...
                      <button
                        title="Manage Status"
                        className="p-3 bg-slate-100 text-slate-600 rounded-xl hover:bg-orange-500 hover:text-white transition-all cursor-pointer"
                      >
                        <ShieldAlert size={20} />
                      </button>
                      <button
                        title="Delete User"
                        className="p-3 bg-red-50 text-red-500 rounded-xl hover:bg-red-500 hover:text-white transition-all cursor-pointer"
                      >
                        <Trash2 size={20} />
                      </button>
                    </div>
