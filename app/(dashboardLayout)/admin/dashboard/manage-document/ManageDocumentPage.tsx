...
export default function ManageDocumentsPage({ docRes }: { docRes: any }) {
  ...
  const getDocTypeIcon = (type: string) => {
    switch (type) {
      case "ASSIGNMENT":
        return <Layers className="text-orange-500" size={24} />;
      case "PRODUCT_DESC":
        return <ShoppingBag className="text-purple-500" size={24} />;
      case "RESUME":
      ...
      <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-xl text-slate-500 font-medium leading-relaxed">
            Monitoring all machine-generated content and token expenditures.
          </p>
        </div>
      </header>
                ...
                >
                    ...
                    <div className="flex items-start gap-5">
                      ...
                      <div>
                        ...
                        <div className="flex items-center gap-2 text-slate-400 font-bold text-base">
                          <span className="bg-slate-100 px-2 py-0.5 rounded text-[12px] text-slate-500 uppercase">
                            {doc?.docType}
                          </span>
                          <span>•</span>
                          <span>
                  ...
                  <td className="p-8">
                    ...
                    <div className="flex items-center gap-2 mt-1">
                      <div
                        className={`w-2 h-2 rounded-full ${doc?.status === "COMPLETED" ? "bg-emerald-500" : "bg-orange-400"}`}
                      />
                      <span className="text-base font-black text-slate-400 uppercase tracking-tighter">
                        {doc?.status}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="p-8 text-center">
                    <div className="flex items-center justify-center gap-3">
                      <button
                        ...
                        onClick={() =>
                          window.open(`/generation-details/${doc.id}`, "_blank")
                        }
                      >
                        <Eye size={20} />
                      </button>
        ...
        {documents?.length === 0 && (
          <div className="py-32 text-center">
            ...
            <p className="text-2xl font-bold text-slate-300 italic">
              No AI generations found in the archive.
            </p>
          </div>
        )}
      </div>
