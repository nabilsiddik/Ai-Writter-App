      ...
      <header className="mb-12 flex flex-col md:flex-row justify-between items-start md:items-end gap-6">
        <div>
          ...
          <p className="text-xl text-slate-500 font-medium leading-relaxed">
            Manage recurring billing cycles and monitor subscription health.
          </p>
        </div>
      </header>
                ...
                >
                  ...
                  <td className="p-8">
                    <div className="flex flex-col gap-2">
                      <div
                        className={`flex items-center gap-2 font-black text-base uppercase tracking-widest ${sub?.plan === "PREMIUM" ? "text-secondary" : "text-primary"}`}
                      >
                        {sub?.plan === "PREMIUM" ? (
                          <Crown size={18} />
                        ) : (
                      ...
                      <div className="flex items-center gap-2 text-slate-700 font-bold text-lg">
                        <div
                          className={`w-2.5 h-2.5 rounded-full ${sub?.status === "ACTIVE" ? "bg-emerald-500" : "bg-red-500"} animate-pulse`}
                        />
                        {sub?.status}
                      </div>
                    </div>
                    ...
                    <div className="space-y-2">
                      <div className="flex items-center gap-2 text-slate-500 font-bold text-base">
                        ...
                        <span className="font-mono text-slate-400">
                          {sub?.stripeSubscriptionId?.substring(0, 12)}...
                        </span>
                      </div>
                      <p className="text-base font-black text-slate-300 uppercase tracking-tighter">
                        Cus: {sub?.stripeCustomerId?.substring(0, 12)}...
                      </p>
                    </div>
                  </td>
