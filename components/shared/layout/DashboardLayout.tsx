...
const ADMIN_MENU: MenuItem[] = [
  ...
  },
  {
    ...
    icon: <FileText size={22} />,
    href: "/admin/dashboard/manage-document",
  },
  {
    label: "Subscriptions",
          ...
          <Link href="/">
            <div className="flex items-center gap-4 mb-12 px-2">
              ...
              {isSidebarOpen && (
                ...
                >
                  Content Writter<span className="text-primary">.ai</span>
                </motion.span>
              )}
            </div>
          </Link>
                  ...
                  {hasSubmenu ? (
                    ...
                    >
                      <div className="flex items-center gap-4 text-gray-400 group-hover:text-white transition-colors">
                        <span
                          className={
                            expandedMenu === item?.label ? "text-primary" : ""
                          }
                        >
                          {item?.icon}
                        </span>
                    ...
                    <Link
                      href={item?.href || "#"}
                      className={`flex items-center gap-4 p-4 rounded-2xl transition-all group ${isActive ? "bg-primary text-white shadow-xl shadow-primary/20" : "text-gray-400 hover:bg-white/5 hover:text-white"}`}
                    >
                    ...
                    {hasSubmenu &&
                      ...
                      isSidebarOpen && (
                        <motion.div
                          ...
                          className="overflow-hidden ml-12 space-y-2 mt-2"
                        >
                          {item?.submenu?.map((sub) => (
                            <Link
                              key={sub?.label}
                              href={sub?.href}
                              className={`block p-3 text-base rounded-xl transition-all ${pathname === sub?.href ? "text-primary font-bold" : "text-gray-500 hover:text-white"}`}
                            >
                              {sub?.label}
                            </Link>
                          ))}
        ...
        <header className="bg-black h-24 border-b border-white/5 flex items-center justify-between px-8 bg-dark/50 backdrop-blur-xl z-50">
          <div className="flex items-center gap-6">
            ...
            <button
              ...
              className="hidden lg:block p-2 text-gray-400 hover:text-white transition-colors cursor-pointer"
            >
